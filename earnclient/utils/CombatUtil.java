...
{
    public static double yawDist(final EntityLivingBase e) {
        if (e != null) {
            ...
            final double d = Math.abs(Minecraft.getMinecraft().thePlayer.rotationYaw - (Math.toDegrees(Math.atan2(difference.zCoord, difference.xCoord)) - 90.0)) % 360.0;
    ...
    public static Object[] getEntityCustom(final float pitch, final float yaw, final double distance, final double expand, final float partialTicks) {
        ...
        final Vec3 var4 = var2.getLookCustom(pitch, yaw, 0.0f);
        final Vec3 var5 = var3.addVector(var4.xCoord * distance, var4.yCoord * distance, var4.zCoord * distance);
        Vec3 var6 = null;
        final float var7 = 1.0f;
        final List var8 = mc.theWorld.getEntitiesWithinAABBExcludingEntity(var2, var2.getEntityBoundingBox().addCoord(var4.xCoord * distance, var4.yCoord * distance, var4.zCoord * distance).expand(1.0, 1.0, 1.0));
    ...
    public static float[] getRotationsToEnt(final Entity ent, final EntityPlayerSP playerSP) {
